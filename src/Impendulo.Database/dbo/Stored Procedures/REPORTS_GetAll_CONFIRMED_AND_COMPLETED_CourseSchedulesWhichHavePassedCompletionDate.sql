CREATE PROCEDURE [dbo].[REPORTS_GetAll_CONFIRMED_AND_COMPLETED_CourseSchedulesWhichHavePassedCompletionDate]
--DROP PROCEDURE [REPORTS_GetAll_CONFIRMED_AND_COMPLETED_CourseSchedulesWhichHavePassedCompletionDate]
AS
...
WHERE        (Schedules.ScheduleStatusID = 2) 
					AND (CONVERT(DATE,Schedules.ScheduleCompletionDate) < (CONVERT(DATE,DATEADD (DAY , 0 , GETDATE() )  )))
					AND LookupEnrollmentProgressStates.LookupEnrollmentProgressStateID = 2
